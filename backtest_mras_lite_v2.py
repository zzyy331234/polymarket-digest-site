...
"""
MRAS-Lite Backtest v2
=====================

在 v1 基础上新增：
- carry_no 过滤收紧
- signal cooldown 去重（token + regime + direction）
- raw / dedup 双口径统计
- 分 regime 输出强化

输出：
- outputs/backtest_mras_lite_v2_summary.json
- outputs/backtest_mras_lite_v2_summary.md
- outputs/backtest_mras_lite_v2_signals_raw.jsonl
- outputs/backtest_mras_lite_v2_signals_dedup.jsonl
"""
...
CARRY_NO_MAX_YES_PRICE = 0.02
CARRY_NO_MIN_LIQUIDITY = 100000
CARRY_NO_MAX_ABS_CHG_24H = 0.02
CARRY_NO_MIN_HOURS_TO_EVENT = 72
CARRY_NO_MAX_NOISE = 2.2
...
def evaluate_mras_lite(feature: Dict) -> Optional[Dict]:
    ...
    # 1) carry_no (v2 tightened)
    if yes_price <= CARRY_NO_MAX_YES_PRICE:
        if liquidity < CARRY_NO_MIN_LIQUIDITY:
            return None
        if chg_24h is not None and abs(chg_24h) > CARRY_NO_MAX_ABS_CHG_24H:
            return None
        if hours_evt is not None and hours_evt < CARRY_NO_MIN_HOURS_TO_EVENT:
            return None
        if noise is not None and noise > CARRY_NO_MAX_NOISE:
            return None
        setup = 0.88 if liquidity >= 300000 else 0.74
        edge = 0.60 if (chg_24h is None or abs(chg_24h) <= 0.01) else 0.48
        execution = 0.80
        ...
        return build_signal(
            ...
            reasons=["ultra low YES price", "carry_no v2 tightened"],
...
def backtest(
    ...
    summary = {
        ...
        "config": {
            ...
            "carry_no_filters": {
                "max_yes_price": CARRY_NO_MAX_YES_PRICE,
                "min_liquidity": CARRY_NO_MIN_LIQUIDITY,
                "max_abs_chg_24h": CARRY_NO_MAX_ABS_CHG_24H,
                "min_hours_to_event": CARRY_NO_MIN_HOURS_TO_EVENT,
                "max_noise": CARRY_NO_MAX_NOISE,
            },
        },
...
def render_markdown(summary: Dict) -> str:
    lines: List[str] = []
    lines.append("# MRAS-Lite 回测结果 v2")
    lines.append("")
    ...
    lines.append(f"- carry_no 过滤：`{json.dumps(cfg['carry_no_filters'], ensure_ascii=False)}`")
    ...
    for bucket in ["raw", "dedup"]:
        lines.append(f"## {bucket.upper()} 信号概览")
        lines.append("")
        sig = summary[bucket]["signals"]
        ...
        lines.append("")
        lines.append(f"## {bucket.upper()} 总体表现")
        lines.append("")
        for label, perf in summary[bucket]["performance"].items():
            ...
            lines.append("")
        lines.append(f"## {bucket.upper()} 分 Regime 表现")
        lines.append("")
        for label, regimes in summary[bucket]["performance_by_regime"].items():
            lines.append(f"### {bucket.upper()} {label}")
            lines.append("")
...
def main() -> None:
    ...
    summary_path = OUTPUT_DIR / "backtest_mras_lite_v2_summary.json"
    md_path = OUTPUT_DIR / "backtest_mras_lite_v2_summary.md"
    signals_raw_path = OUTPUT_DIR / "backtest_mras_lite_v2_signals_raw.jsonl"
    signals_dedup_path = OUTPUT_DIR / "backtest_mras_lite_v2_signals_dedup.jsonl"

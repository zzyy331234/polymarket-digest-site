...
"""
MRAS-Lite Backtest v3
=====================

在 v2 基础上新增：
- 移除 carry_no，转为 mean_revert-first
- trend 增加 8h horizon 对比
- raw / dedup 双口径统计
- 增加 strategy slices：mean_revert_only / trend_only / no_carry

输出：
- outputs/backtest_mras_lite_v3_summary.json
- outputs/backtest_mras_lite_v3_summary.md
- outputs/backtest_mras_lite_v3_signals_raw.jsonl
- outputs/backtest_mras_lite_v3_signals_dedup.jsonl
"""
...
def evaluate_mras_lite_v3(feature: Dict) -> Optional[Dict]:
    ...
    mr_reasons: List[str] = []
    mr_strength = 0
    if pct_rank is not None and (pct_rank <= 0.08 or pct_rank >= 0.92):
        mr_strength += 1
        mr_reasons.append("recent percentile extreme")
    if rsi is not None and (rsi <= 33 or rsi >= 67):
        mr_strength += 1
        mr_reasons.append("RSI extreme")
    if chg_24h is not None and abs(chg_24h) >= 0.035:
        mr_strength += 1
        mr_reasons.append("24h move extended")
    if noise is not None and noise <= 2.2:
        mr_strength += 1
        mr_reasons.append("noise acceptable")

    if pct_rank is not None and mr_strength >= 3:
        direction = None
        if pct_rank <= 0.08 and yes_price <= 0.45:
            direction = "YES"
            mr_reasons.insert(0, "recent lower percentile bounce")
        elif pct_rank >= 0.92 and yes_price >= 0.55:
            direction = "NO"
        ...
        if direction:
            ...
            if rsi is not None and (rsi <= 28 or rsi >= 72):
                edge += 0.08
            if chg_24h is not None and abs(chg_24h) >= 0.06:
                edge += 0.05
    ...
    if chg_4h is not None and abs(chg_4h) >= 0.025:
        trend_hits += 1
    ...
    if noise is not None and noise <= 2.0:
        trend_hits += 1
        reasons.append("noise controlled")

    if trend_hits >= 5 and chg_24h is not None:
        direction = "YES" if chg_24h > 0 else "NO"
        setup = clamp(0.58 + (0.12 if liquidity >= 150000 else 0.0))
        edge = clamp(0.45 + min(abs(chg_24h) * 2.0, 0.22))
        execution = 0.70
    ...
    if yes_price >= 0.70 and chg_24h is not None and chg_24h < -0.04:
        edge = 0.55
...
def dedupe_signals(items: List[Dict], cooldown_hours: float) -> List[Dict]:
    cooldown_seconds = cooldown_hours * 3600.0
    last_seen: Dict[Tuple[str, str, str], datetime] = {}
    out: List[Dict] = []
    for row in sorted(items, key=lambda x: (x["entry_ts"], -x["confidence"])):
        key = (row["token_id"], row["regime"], row["direction"])
        ts = datetime.fromisoformat(row["entry_ts"])
        prev = last_seen.get(key)
        if prev is not None and (ts - prev).total_seconds() < cooldown_seconds:
            continue
        last_seen[key] = ts
        out.append(row)
    return out
...
def pack_stats(items: List[Dict]) -> Dict:
    regime_counter = Counter(row["regime"] for row in items)
    position_counter = Counter(row["position"] for row in items)
    direction_counter = Counter(row["direction"] for row in items)
    return {
        "signals": {
            "total": len(items),
        ...
        "performance": {label: summarize_horizon(items, label) for label in HORIZONS},
        "performance_by_regime": {label: summarize_by_regime(items, label) for label in HORIZONS},
    }


def build_slices(items: List[Dict]) -> Dict[str, List[Dict]]:
    return {
        "all": items,
        "no_carry": [row for row in items if row["regime"] != "carry_no"],
        "mean_revert_only": [row for row in items if row["regime"] == "mean_revert"],
        "trend_only": [row for row in items if row["regime"] == "trend"],
        "contrarian_only": [row for row in items if row["regime"] == "contrarian"],
    }
...
def backtest(
    ...
    for idx_market, meta in enumerate(markets, start=1):
        ...
        for seg in segments.values():
            ...
            for i in range(96, len(seg)):
                ...
                if not feature:
                    continue
                signal = evaluate_mras_lite_v3(feature)
    ...
    dedup_rows = dedupe_signals(all_rows, cooldown_hours=cooldown_hours)
    raw_slices = build_slices(all_rows)
    dedup_slices = build_slices(dedup_rows)
    ...
    summary = {
        ...
        "raw": pack_stats(all_rows),
        "dedup": pack_stats(dedup_rows),
        "raw_slices": {name: pack_stats(rows) for name, rows in raw_slices.items()},
        "dedup_slices": {name: pack_stats(rows) for name, rows in dedup_slices.items()},
        "top_examples": sorted(
            dedup_rows,
            key=lambda x: (
                x["confidence"],
                x.get("outcomes", {}).get("24h", {}).get("roi", x.get("outcomes", {}).get("12h", {}).get("roi", x.get("outcomes", {}).get("8h", {}).get("roi", x.get("outcomes", {}).get("4h", {}).get("roi", -999)))),
            ),
            reverse=True,
        )[:20],
    }
...
def render_slice(lines: List[str], title: str, stats: Dict) -> None:
    lines.append(f"## {title} 信号概览")
    lines.append("")
    sig = stats["signals"]
    ...
    lines.append("")
    lines.append(f"## {title} 总体表现")
    lines.append("")
    for label, perf in stats["performance"].items():
        ...
        lines.append("")
    lines.append(f"## {title} 分 Regime 表现")
    lines.append("")
    for label, regimes in stats["performance_by_regime"].items():
        lines.append(f"### {title} {label}")
        lines.append("")
...
def render_markdown(summary: Dict) -> str:
    lines: List[str] = []
    lines.append("# MRAS-Lite 回测结果 v3")
    lines.append("")
    ...
    lines.append(f"- cooldown：`{cfg['cooldown_hours']}` 小时")
    lines.append(f"- carry_no_enabled：`{cfg['carry_no_enabled']}`")
    lines.append(f"- strategy_focus：`{cfg['strategy_focus']}`")
    lines.append("")
    lines.append("## 数据质量")
    lines.append("")
    q = summary["quality"]
    ...
    render_slice(lines, "RAW", summary["raw"])
    render_slice(lines, "DEDUP", summary["dedup"])

    for group_name in ["raw_slices", "dedup_slices"]:
        lines.append(f"# {group_name.upper()}")
        lines.append("")
        for slice_name, stats in summary[group_name].items():
            render_slice(lines, f"{group_name.upper()} {slice_name}", stats)

    lines.append("## Top Examples")
    lines.append("")
    for row in summary["top_examples"][:10]:
        best_roi = row['outcomes'].get('24h', row['outcomes'].get('12h', row['outcomes'].get('8h', row['outcomes'].get('4h', {})))).get('roi')
        lines.append(
            f"- [{row['regime']}] {row['direction']} conf={row['confidence']} best_roi={best_roi} | {row['question']} | {row['entry_ts']}"
        )
    lines.append("")
    return "\n".join(lines)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="MRAS-Lite 回测 v3")
    ...
    rows_dedup = result["rows_dedup"]

    summary_path = OUTPUT_DIR / "backtest_mras_lite_v3_summary.json"
    md_path = OUTPUT_DIR / "backtest_mras_lite_v3_summary.md"
    signals_raw_path = OUTPUT_DIR / "backtest_mras_lite_v3_signals_raw.jsonl"
    signals_dedup_path = OUTPUT_DIR / "backtest_mras_lite_v3_signals_dedup.jsonl"

    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2)
    with open(md_path, "w", encoding="utf-8") as f:
        f.write(render_markdown(summary))
    with open(signals_raw_path, "w", encoding="utf-8") as f:
        for row in rows_raw:
            f.write(json.dumps(row, ensure_ascii=False) + "\n")
    with open(signals_dedup_path, "w", encoding="utf-8") as f:
        for row in rows_dedup:
            f.write(json.dumps(row, ensure_ascii=False) + "\n")

    print(f"raw_signals={len(rows_raw)} dedup_signals={len(rows_dedup)} summary={summary_path} md={md_path}")
    for bucket in ["raw", "dedup"]:
        for horizon, perf in summary[bucket]["performance"].items():
            print(
                f"{bucket}:{horizon}: count={perf['count']} hit_rate={perf['hit_rate']} avg_roi={perf['avg_roi']} max_consecutive_losses={perf['max_consecutive_losses']}"
            )
    for slice_name in ["mean_revert_only", "trend_only", "contrarian_only"]:
        stats = summary["dedup_slices"][slice_name]["performance"]
        for horizon, perf in stats.items():
            print(
                f"dedup_slice:{slice_name}:{horizon}: count={perf['count']} hit_rate={perf['hit_rate']} avg_roi={perf['avg_roi']}"
            )
